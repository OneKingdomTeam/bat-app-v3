...
def add_default_questions():
    ...
    for i in range(0, 13):
        for key, val in questions[i].items():
            ...
            category_row_id = data.load_category(
                category_name=category_name, category_order=i
            )
            print("\n")
            for q_id in range(1, 5):
                ...
                if (
                    questions[i][key]["segment1"][f"question{q_id}"]["commentTitle"]
                    != ""
                ):
                    question_description = questions[i][key]["segment1"][
                        f"question{q_id}"
                    ]["commentTitle"]
                if (
                    questions[i][key]["segment1"][f"question{q_id}"][
                        "commentDescription"
                    ]
                    != ""
                ):
                ...
                question_no = questions[i][key]["segment1"][f"question{q_id}"]["radio"][
                    "option1"
                ]
                question_mid = questions[i][key]["segment1"][f"question{q_id}"][
                    "radio"
                ]["option2"]
                question_yes = questions[i][key]["segment1"][f"question{q_id}"][
                    "radio"
                ]["option3"]
                print(
                    f"{question}\n{question_description}\n{question_no}\n{question_yes}\n{question_mid}"
                ...
                data.load_question(
                    ...
                    question_order=q_id,
                    option_no=question_no,
                    option_yes=question_yes,
                    option_mid=question_mid,
...
def get_all_questions_for_category(
    category_id: int, current_user: User
) -> list[Question]:
...
def rename_questions_category(
    ...
    if category_id_from_path != category_rename.category_id:
        raise EndpointDataMismatch(
            msg="Rename data doesn't match you endpoint. Try again. Or let us know."
        )
...
def reorder_questions_category(
    questions_category_reorder: QuestionCategoryReorder, current_user: User
) -> bool:
    ...
    for category_reorder_item in questions_category_reorder.reorder_data:
        update_statuses.append(data.reorder_questions_category(category_reorder_item))
        if False in update_statuses:
            return False
...
def update_question_content(
    question_edit_content: QuestionEditContent, current_user: User
) -> Question:
