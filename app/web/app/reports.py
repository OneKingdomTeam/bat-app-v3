...
@router.get("/{assessment_id}", response_class=HTMLResponse, name="app_reports_for_assessment")
def get_reports_for_assessment(request: Request, assessment_id: str, current_user: User = Depends(user_htmx_dep)):

    assessment = assessment_service.get_for_user(assessment_id=assessment_id, current_user=current_user)
    reports = service.get_public_reports_for_assessment(assessment_id=assessment_id, current_user=current_user)
