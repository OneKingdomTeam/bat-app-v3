...
def get_questions_reorder_category(request: Request, current_user: User = Depends(user_htmx_dep)):
    ...
    response = jinja.TemplateResponse(
            name="dashboard/questions-reorder.html",
            context=context
            )
...
def post_questions_reorder_category(category_new_order: QuestionCategoryReorder, request: Request, current_user: User = Depends(user_htmx_dep)):
    ...
    try:
        service.reorder_questions_category(questions_category_reorder=category_new_order, current_user=current_user)
    except:
        raise
    return ""
...
def get_question_category_rename(category_id: int, request: Request, current_user: User = Depends(user_htmx_dep)):
    ...
    questions_categories: list[QuestionCategory] = service.get_all_categories(current_user=current_user)
    questions_in_category: list[Question] = service.get_all_questions_for_category(category_id=category_id, current_user=current_user)
    category_for_edit: QuestionCategory = service.get_questions_category(category_id=category_id, current_user=current_user)
    ...
    response = jinja.TemplateResponse(
            context=context,
            name="dashboard/questions-rename.html"
            )
...
def post_question_category_rename(category_id: int, category_rename: QuestionCategoryRename, request: Request, response: Response,  current_user: User = Depends(user_htmx_dep)):
    ...
    try: 
        service.rename_questions_category(category_id_from_path=category_id, category_rename=category_rename, current_user=current_user)
    except:
       # NotImplemented
       raise 
...
def put_question_category_page(category_id: int, question_id: int, \
        question_edit_content: QuestionEditContent, request: Request, \
...
@router.get("/{category_id}", response_class=HTMLResponse, name="dashboard_question_category_page")
def get_question_category_page(category_id: int, request: Request, current_user: User = Depends(user_htmx_dep)):
